...
def build_upper(cfg, layout):
    ...
    # lay-c: 인장을 맨 위로 분리 → 사진 → 나머지 타이틀(곡명·부제·아티스트)
    if layout == "lay-c":
        titles_no_stamp = (
            '    <div class="titles">\n'
            f'      <h1>{song_html}</h1>\n'
        ...
        stamp_top = series_html.replace("      ", "    ", 1).rstrip("\n")
        return f"{stamp_top}\n\n{plate}\n\n{titles_no_stamp}"
    # B는 제목이 먼저, A는 자켓이 먼저 (여름밤 04 그대로)
...
def build_note(text, credit=""):
    ...
    if text:
        ...
        for p in paras:
            ...
            elif p.startswith("[가사]"):
                # 가사/시 인용 — 왼쪽 세로줄. 줄바꿈은 원문 그대로 유지.
                body = "<br>".join(esc(l) for l in p[4:].strip().split("\n"))
                out += f'<div class="lyric">{body}</div>'
            else:
                out += f"<p>{esc(p)}</p>"
    if credit:
        def fmt_credit_line(l):
            l = l.strip()
            parts = l.split(" ", 1)
            if len(parts) == 2:
                label, rest = parts
                if " · " in rest:
                    name_items = rest.split(" · ")
                    wrapped = '<span class="cr-sep"> · </span>'.join(f'<span class="cr-name">{esc(n.strip())}</span>' for n in name_items)
                else:
                    wrapped = f'<span class="cr-name">{esc(rest.strip())}</span>'
                return f'<span class="cr-line"><span class="cr-label">{esc(label)}</span><span class="cr-names">{wrapped}</span></span>'
            return f'<span class="cr-line">{esc(l)}</span>'
        lines = "".join(fmt_credit_line(l) for l in credit.split("\n") if l.strip())
...
def build_film(yt_id, caption, mode="embed"):
    if not yt_id:
        return ""
    # link 모드: 임베드가 막힌 영상(레이블 차단 등) — 썸네일 + 재생버튼, 클릭 시 유튜브로 이동
    if mode == "link":
        thumb = f"https://img.youtube.com/vi/{esc(yt_id)}/maxresdefault.jpg"
        watch = f"https://www.youtube.com/watch?v={esc(yt_id)}"
        return (
            '    <figure class="film">\n'
            '      <div class="mat">\n'
            f'        <a class="frame film-link" href="{watch}" target="_blank" rel="noopener" '
            f'aria-label="{esc(caption)} — 유튜브에서 보기">\n'
            f'          <img class="film-thumb" src="{thumb}" alt="{esc(caption)}" loading="lazy">\n'
...
def main(cfg_path):
    ...
    html = (tpl
        ...
        .replace("{{FILM}}", build_film(cfg.get("youtube"), cfg.get("caption", ""), cfg.get("film_mode", "embed")))
